...
class SubCategory extends StatefulWidget {
  SubCategory({Key? key, required this.id, required this.title})
      : super(key: key);
  final int id;
  final String title;
...
class _SubCategoryState extends State<SubCategory> {
  ...
  Future<SubCategoryModel?> getSubCategory() async {
    try {
      ...
      var response =
          await Http.get(Uri.parse(subCategoryUrl + widget.id.toString()));
      print("12121121${response.body}");
      if (response.statusCode == 200) {
        setState(() {
          _subCategoryModel =
              SubCategoryModel.fromJson(jsonDecode(response.body));
          loader = false;
        });
        print("0000000212$_subCategoryModel");
      }
                            ...
                            MaterialPageRoute(
                                builder: (context) =>
                                    CategoryProductProductDetailsPage(
                                      ...
                                      proid:
                                          dataList.value[index].id.toString(),
                                      title: dataList.value[index].brandName,
                                    )));
                      },
                      child: Column(
                        children: [
                          Container(
                            ...
                            child: ClipRRect(
                              borderRadius: BorderRadius.circular(10),
                              child: Image.network(
                                "https://dealsbuck.com/${dataList.value[index].brandImagePath}",
  ...
  void getdata() async {
    var response =
        await ApiConfig().getParticularCategoryList(widget.id.toString());
    if (response != null) {
